...
export class StudentHomeComponent {
  ...
  constructor(private submissionService: SubmissionService, private activatedRouter: ActivatedRoute, private langSevice: LanguageService, private contestService: ContestService, private messageService: MessageService) {
  ...
  ngOnInit() {
    ...
    this.submissionService.getAllSubmissions('', this.contestId).subscribe((data) => {
      for (let i = 0; i < data.length; i++) {
  ...
  submitCode() {
    if (this.selProblem && this.selectedLanguage && this.code.length > 0) {
      ...
      this.submissionService.createSubmission(file, this.contestId, this.selectedLanguage.id, this.selProblem.id, '7').subscribe();
