...
public class DockerContainerC extends DockerContainer{
    ...
    public Result ejecutar(String imagenId) throws IOException {
        ...
        CreateContainerResponse container = dockerClient.createContainerCmd(imagenId).withNetworkDisabled(true).withEnv("EXECUTION_TIMEOUT="+result.getMaxTimeout(), "FILENAME2="+nombreClase+".c" ).withHostConfig(hostConfig).withName(nombreDocker).exec();
